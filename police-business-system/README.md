...
- **業務總覽改成兩層式卡片**：`index.html` 不再用巢狀清單呈現業務樹，第一層是
  「承辦人總覽」——每位承辦人一張卡片（頭像色塊、負責幾項業務、業務標籤、目前
  最需要注意的敘獎狀態），點進去才是第二層——那個人負責的業務卡片清單（沿用
  麵包屑、敘獎狀態燈號、新增子項目功能）。新的 `GET /api/nodes/cards` 端點一次
  回傳每張業務卡片需要的所有資料（業務大類麵包屑、主辦人含顏色、協辦人姓名、
  最新一期敘獎狀態），避免前端要為每張卡片各自發好幾個請求。
